...
import { ShoppingCart, Plus, Minus, CreditCard, Package } from 'lucide-react';
...
const POSTerminal = ({ products, onCheckoutComplete }: POSTerminalProps) => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setInvoice({ items: [...cart], invoiceNumber: generateInvoiceNumber(), date: new Date() });
      setCart([]);
      onCheckoutComplete();
    } catch {
    ...
    <>
      <div className="p-6 grid md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          {checkoutError && (
            <p className="mb-2 text-xs font-mono text-red-400 text-center">{checkoutError}</p>
          )}

          <button
            onClick={handleCheckout}
            disabled={cart.length === 0 || isProcessing}
            className="w-full py-3 font-mono font-medium rounded-lg flex items-center justify-center gap-2 transition-all bg-terminal-pink text-white hover:shadow-lg hover:shadow-terminal-pink/20 disabled:opacity-50"
          >
            {isProcessing ? (
